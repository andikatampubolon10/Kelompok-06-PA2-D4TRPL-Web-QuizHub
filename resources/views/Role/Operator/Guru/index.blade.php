        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100">
                ...
                <div class="px-6 py-4 border-b border-gray-100 bg-gray-50">
                    ...
                    <form method="GET" action="{{ route('Operator.Guru.index') }}"
                        class="mb-6 flex flex-wrap items-center gap-4">
                        <div>
                            <label for="mata_pelajaran" class="text-sm font-medium text-gray-700">
                                Filter Mata Pelajaran:
                            </label>
                            <select name="mata_pelajaran" id="mata_pelajaran"
                                class="border-gray-300 rounded-lg shadow-sm text-sm focus:ring-green-500 focus:border-green-500">
                                <option value="">Semua Mata Pelajaran</option>
                                @foreach ($mataPelajarans as $mapel)
                                    <option value="{{ $mapel->id_mata_pelajaran }}"
                                        {{ $filterMapel == $mapel->id_mata_pelajaran ? 'selected' : '' }}>
                                        ...
                                        <div class="flex-1">
                                            <div class="flex items-start space-x-4">
                                                ...
                                                <div class="flex-1">
                                                    ...
                                                    {{-- ✅ Tambahan: daftar mata pelajaran --}}
                                                    <div class="mt-3">
                                                        <h5 class="text-sm font-semibold text-gray-700 mb-1">Mata Pelajaran
                                                            Diampu:</h5>
                                                        @if ($teacher->mataPelajarans->isNotEmpty())
                                                            <ul class="list-disc list-inside text-gray-600 text-sm">
                                                                @foreach ($teacher->mataPelajarans as $mapel)
                                                                    <li>{{ $mapel->nama_mata_pelajaran }}</li>
                                                                @endforeach
                                                            </ul>
                                                        @else
                                                            <p class="text-gray-500 text-sm italic">Belum ada mata pelajaran
                                                                yang diampu</p>
                                                        @endif
                                                    </div>
                                                </div>
