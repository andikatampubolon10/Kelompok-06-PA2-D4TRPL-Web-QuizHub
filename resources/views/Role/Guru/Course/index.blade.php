        ...
        @foreach ($ujians->sortBy('tanggal_ujian') as $exam)
            ...
            <div class="flex justify-between items-center mb-4">
                ...
                <div class="flex space-x-5 justify-end items-center">
                    <!-- Tombol Bobot (baru) - sejajar, ikon + teks kuning, tanpa background kuning -->
                    <a href="{{ route('Guru.BobotTipeSoal.index', ['id_ujian' => $exam->id_ujian]) }}"
                        class="inline-flex items-center text-yellow-400 hover:text-yellow-500 px-3 py-2 rounded">
                        <i class="fas fa-balance-scale mr-1 text-yellow-400"></i>
                        <span class="text-yellow-400">Bobot</span>
                    </a>

                    <!-- Tombol Hapus -->
                    <form action="{{ route('Guru.Ujian.destroy', $exam->id_ujian) }}" method="POST"
                        onsubmit="return confirm('Apakah Anda yakin ingin menghapus soal ini?');" class="inline-block">
                        @csrf
                        @method('DELETE')
