    ...
    <form action="{{ route('Guru.Latihan.store') }}" method="POST">
            ...
            <select name="id_mata_pelajaran" id="id_mata_pelajaran" class="block w-full p-2 border border-gray-300 rounded-md">
                ...
                @foreach ($mataPelajarans as $mp)
                    <option value="{{ $mp->id_mata_pelajaran }}" 
                        {{ old('id_mata_pelajaran') == $mp->id_mata_pelajaran ? 'selected' : '' }}>
        ...
        <div class="mb-6">
            ...
            <select name="id_kelas" class="block w-full p-2 border border-gray-300 rounded-md">
                ...
                @foreach ($kelas as $class)
                    <option value="{{ $class->id_kelas }}" {{ old('id_kelas') == $class->id_kelas ? 'selected' : '' }}>
                        {{ $class->nama_kelas }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0 pt-6 border-t border-gray-100 mt-8">
            ...
            <div class="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3">
                <a href="{{ route('Guru.Latihan.index') }}" class="inline-flex items-center justify-center px-6 py-3 border border-gray-300 text-gray-700 bg-white rounded-lg hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 transition-colors">
